...
const PortfoliosBoardsSection: React.FC = () => {
  ...
  const config = useMemo(
    () => getBoardSectionConfigs(t as TranslateFn).portfolio,
    [t]
  );
  const {
    data: boards,
  ...
  const handleCreateBoard = useCallback(
    async (data: { name: string }) => {
      try {
        ...
        logger.error('PortfoliosBoardsSection', 'Failed to create board', err);
        showErrorToast(t('mainPage.createBoardError'));
        throw err;
      }
    },
    [createBoardMutation, trackEvent, config, t, router]
  );

  const ViewComponent = viewMode === 'grid' ? IdeasGridView : IdeasListView;
...
export default PortfoliosBoardsSection;
